...
class AnalyticsOnlyStrategy(CompositionStrategy):
    """Strategy that filters context blocks to only include analytics data."""
    
    def apply(self, context: JourneyIntelligenceContext) -> JourneyIntelligenceContext:
        return dataclasses.replace(
            context,
            blocks=[b for b in context.blocks if b.block_type == "analytics"]
        )
