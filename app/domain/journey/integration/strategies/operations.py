...
class OperationsStrategy(CompositionStrategy):
    """Strategy that filters or summarizes context blocks for an operations view."""
    
    def apply(self, context: JourneyIntelligenceContext) -> JourneyIntelligenceContext:
        return dataclasses.replace(
            context,
            blocks=[b for b in context.blocks if b.block_type in ("operations", "analytics", "journey")]
        )
