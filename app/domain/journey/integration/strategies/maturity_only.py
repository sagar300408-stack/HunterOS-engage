...
class MaturityOnlyStrategy(CompositionStrategy):
    """Strategy that filters context blocks to only include maturity-specific data."""
    
    def apply(self, context: JourneyIntelligenceContext) -> JourneyIntelligenceContext:
        return dataclasses.replace(
            context,
            blocks=[b for b in context.blocks if b.block_type == "maturity"]
        )
