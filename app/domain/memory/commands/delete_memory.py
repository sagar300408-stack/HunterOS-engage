"""
HunterOS Engage — Delete Memory Command Handler (Soft Delete)

Orchestrates soft deletion of CustomerMemory Aggregate Root.
"""
...
from app.domain.memory.commands.models import DeleteMemoryCommand
from app.domain.memory.events.models import MemoryDeletedDomainEvent, NIL_UUID
...
class DeleteMemoryHandler:
    """Handles DeleteMemoryCommand (Soft Delete) within transactional Unit of Work."""
    ...
    async def handle(
        ...
        memory.soft_delete(reason=command.reason, actor=command.actor)
        ...
        await self.audit_service.record_deletion_audit(
            session=session,
            memory=memory,
            actor=command.actor,
        ...
        domain_event = MemoryDeletedDomainEvent(
