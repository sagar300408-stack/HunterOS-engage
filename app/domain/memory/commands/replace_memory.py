"""
HunterOS Engage — Replace Memory Command Handler (PUT)

Orchestrates full replacement of CustomerMemory Aggregate Root state.
Enforces optimistic concurrency control and full version snapshotting.
"""
...
from app.domain.memory.commands.models import ReplaceMemoryCommand
...
class ReplaceMemoryHandler:
    """Handles ReplaceMemoryCommand (PUT) within transactional Unit of Work."""
    ...
    async def handle(
        ...
        # 3. Mutate Aggregate State via Domain Method
        old_payload = memory.replace_payload(
            new_payload=command.memory_payload,
